...
class PostType {
	 ...
	 *
	 * @param array $name The name of the desired post type.
	...
	public function registerPostType() {
		foreach ( $this->customPostType as $postType => $postTypeArgs ) {
			...
			$args         = [
				...
				'rewrite'             => [ 'slug' => strtolower( str_replace( ' ', '-', $singularName ) ) ],
					...
					$options      = array_merge(
						[
							'labels'            => [
								...
								'separate_items_with_commas' => sprintf( __( 'Separate %s with commas', '_mok' ), $pluralName ),
								'add_or_remove_items'        => sprintf( __( 'Add or remove %s', '_mok' ), $pluralName ),
								'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s', '_mok' ), $pluralName ),
