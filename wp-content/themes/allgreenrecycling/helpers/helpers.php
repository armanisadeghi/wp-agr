...
function mok_leads_initial_save( $partial_entry, $form ) {
	...
	try {
		...
		$first_name   = array_key_exists( '75', $partial_entry ) ? $partial_entry['75'] : '';
		$last_name    = array_key_exists( '76', $partial_entry ) ? $partial_entry['76'] : '';
...
function mok_leads_final_save( $entry, $form, $field ) {
	...
	echo'Field: <br>';
	echo'<pre>';print_r($field);echo'</pre><br>';exit; */
	// echo'<pre>';print_r($form);echo'</pre><br>';
	...
	// echo'<pre>';print_r($lines);echo'</pre><br>';
...
/* echo $_SERVER['REQUEST_URI']; */
	...
	if ( $_array['Name'] ) {
		...
		$nameParts = explode(' ', $_array['Name'], 2); // Limit to 2 parts

		// Step 3: Assign the first and last names
	...
	$_http_referrer        = isset($_array['HTTP REFERRER']) ? $_array['HTTP REFERRER'] : '';
	$_referrer_url         = isset($_array['Referrer URL']) ? $_array['Referrer URL'] : '';
	$_referrer_url         = isset($_array['referal_url']) ? $_array['referal_url'] : $_referrer_url;
	$_referrer_url         = ( $_referrer_url === '' ) ? site_url().$_SERVER['REQUEST_URI'] : $_referrer_url;	
	...
	try {
		...
		// fwrite($myfile, "1.".$_array['Televisions / Monitors']."\n");
		// fwrite($myfile, "2.".$rs_itad_tv_monitors."\n");
		// fwrite($myfile, "3.".corresponding_combo_value( $rs_itad_tv_monitors )."\n");
		// fclose($myfile);
		...
		/* $leads_db = new PDO(
			// 'mysql:host=127.0.0.1;dbname=wp_allgreenrecyc;charset=utf8mb4', 'allgreenrecyc', 'zbF7JpxMC0loskI4guHk', [
			'mysql:host=127.0.0.1;dbname=i9324173_wp2;charset=utf8mb4', 'i9324173_wp2', 'uQ?j&T$&BM!M}b4n', [
				PDO::ATTR_EMULATE_PREPARES => false,
				PDO::ATTR_ERRMODE          => PDO::ERRMODE_EXCEPTION,
			]
		); */
