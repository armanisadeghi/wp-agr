...
use BracketSpace\Notification\Dependencies\Composer\Cache;
...
use BracketSpace\Notification\Dependencies\Composer\Exception\IrrecoverableDownloadException;
use BracketSpace\Notification\Dependencies\Composer\Package\Comparer\Comparer;
...
use BracketSpace\Notification\Dependencies\Composer\Plugin\PostFileDownloadEvent;
use BracketSpace\Notification\Dependencies\Composer\Plugin\PreFileDownloadEvent;
...
use BracketSpace\Notification\Dependencies\Composer\Util\Http\Response;
...
use BracketSpace\Notification\Dependencies\Composer\Util\Url as UrlUtil;
    ...
    public function download(PackageInterface $package, string $path, ?PackageInterface $prevPackage = null, bool $output = true): PromiseInterface
    {
        ...
        $download = function () use ($output, $cacheKeyGenerator, $package, $fileName, &$urls, &$accept, &$reject) {
            ...
            if ($this->cache !== null && ($checksum === null || $checksum === '' || $checksum === $this->cache->sha1($cacheKey)) && $this->cache->copyTo($cacheKey, $fileName)) {
                ...
                $result = \BracketSpace\Notification\Dependencies\React\Promise\resolve($fileName);
            } else {
                if ($output) {
