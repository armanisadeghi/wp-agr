...
use BracketSpace\Notification\Dependencies\Composer\Config\JsonConfigSource;
...
use BracketSpace\Notification\Dependencies\Composer\Package\Archiver;
use BracketSpace\Notification\Dependencies\Composer\Package\Version\VersionGuesser;
...
class Factory
{
    ...
    public function createComposer(IOInterface $io, $localConfig = null, $disablePlugins = false, ?string $cwd = null, bool $fullLoad = true, bool $disableScripts = false)
    {
        ...
        if ($fullLoad) {
            ...
            if (false === $disablePlugins && false === $disableScripts && !class_exists('BracketSpace\Notification\Dependencies\Composer\InstalledVersions', false) && file_exists($installedVersionsPath = $config->get('vendor-dir').'/composer/installed.php')) {
                ...
                if (class_exists('BracketSpace\Notification\Dependencies\Composer\InstalledVersions')) {
        ...
        $package = $loader->load($localConfig, 'BracketSpace\Notification\Dependencies\Composer\Package\RootPackage', $cwd);
