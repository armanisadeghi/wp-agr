...
namespace BracketSpace\Notification\Dependencies\BrianHenryIE\Strauss\Composer\Extra;

use BracketSpace\Notification\Dependencies\Composer\Composer;
use Exception;
use BracketSpace\Notification\Dependencies\JsonMapper\JsonMapperFactory;
...
class StraussConfig
{
    ...
    public function __construct(Composer $composer)
    {
        ...
        if (!is_null($configExtraSettings)) {
            ...
            $mapper->push(new \BracketSpace\Notification\Dependencies\JsonMapper\Middleware\CaseConversion(
                \BracketSpace\Notification\Dependencies\JsonMapper\Enums\TextNotation::UNDERSCORE(),
                \BracketSpace\Notification\Dependencies\JsonMapper\Enums\TextNotation::CAMEL_CASE()
            ));
        ...
        if (empty($this->packages)) {
            $this->packages = array_map(function (\BracketSpace\Notification\Dependencies\Composer\Package\Link $element) {
