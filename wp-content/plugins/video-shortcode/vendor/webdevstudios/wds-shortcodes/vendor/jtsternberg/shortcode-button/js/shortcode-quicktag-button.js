...
( function( window, document, $, wp, QTags, l10n, btns, undefined ) {
	...
	// wp_localize_script may produce a plain object if PHP array keys are non-sequential.
	if ( ! Array.isArray( l10n ) && typeof l10n === 'object' ) {
		l10n = Object.keys( l10n ).sort( function( a, b ) {
	...
	btns.init = function() {
		...
		for ( var i = l10n.length - 1; i >= 0; i-- ) {
			...
			// text editor quicktags button (optional: not loaded in some admin contexts)
			...
			// Block editor and other screens may not load TinyMCE views — do not fatal the script.
			if ( button.params.mceView && wp.mce && wp.mce.views && typeof wp.mce.views.register === 'function' ) {
	...
	btns.jQueryInit = function() {
		...
		// Bind UI handlers before btns.init() so a JS error there cannot strand an open modal.
		$c.body
